...
var appState = {
  ...
  sceneMarkers: {}               // sceneIdx -> [{id, materialId, label, x, y}]
};
...
function detectSheetLayout(data){
  ...
  for (let r=0; r<scan; r++){
    ...
    if (score >= 2){
      const width = Math.max(area, item, remarks, image);
...
function cleanRemarks(val){
  const t = trim(val);
  if (!t) return '';
  if (/^\s*(REMARKS|비고)\s*$/i.test(t)) return '';
  if (/이미지\s*첨부|도어\s*모델링/i.test(t)) return '';
  return t;
}
...
function handleExcelUpload(e){
  ...
  reader.onload = function(ev){
    try{
      ...
      if(info){ info.innerHTML = '<strong>업로드 완료:</strong> '+file.name+' ('+wb.SheetNames.length+'개 시트)'; info.style.display = 'block'; }
...
function autoSelectFirstValidSheet(){
  ...
  if(valid.length){
    ...
    try { parseExcelData(); }
    catch (err) { console.error('parseExcelData error:', err); showStatus('엑셀 파싱 중 오류: '+ err.message, 'error'); }
  }
}
...
function parseExcelData(){
  ...
  function robustPickImage(data, r, imageCol){
    const pick = (val) => { const url = extractImageUrl(val); return isLikelyImage(url) ? url : ''; };
    const row = data[r] || [];
    ...
    for (let k=1; k<=2; k++){
      const r2 = r + k; if (r2 >= data.length) break;
      const row2 = data[r2] || [];
      if (imageCol > -1) {
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    for (var r = 1; r < data.length; r++) {
      ...
      if (hit.key === 'AREA') {
        ...
        if (mv && !isHeaderNoise(mv)) current.material = mv;
...
function displaySceneInfo(){
  ...
  html+='</div>';
  var el=document.getElementById('sceneInfo'); if(el){ el.innerHTML=html; el.style.display='block'; }
  setTimeout(checkAllFilesUploaded,100);
}
...
function checkAllFilesUploaded(){
  ...
  if(hasExcel && hasMinimap && hasScenes){
    try{
      ...
      document.getElementById('matchingStep') && (document.getElementById('matchingStep').style.display='block');
      document.getElementById('generateStep') && (document.getElementById('generateStep').style.display='block');
      document.getElementById('minimapDrawWrap') && (document.getElementById('minimapDrawWrap').style.display='block');
      showStatus('모든 파일이 업로드되었습니다! 장면별 자재와 미니맵 위치를 지정해주세요.','success');
      // 어노테이터 최신 장면 표시
      renderSceneBadges(appState.currentSelectedScene||0);
...
function createSceneSelector(){
  var c=document.getElementById('sceneSelector'); if(!c) return; c.innerHTML='';
...
function createMaterialTabFilter(){
  var c=document.getElementById('materialTabButtons'); if(!c) return; c.innerHTML='';
...
function createMaterialTable(){
  var c=document.getElementById('materialTableList'); if(!c) return; c.innerHTML='';
  ...
  appState.materials.forEach(function(m, i){
    ...
    // No. 셀: 드래그 소스
    var noCellHtml =
      '<td draggable="true" ondragstart="handleDragStart(event, '+m.id+','+(i+1)+')"' +
      ' style="border:1px solid #ddd;padding:4px;text-align:center;font-size:0.8em;cursor:grab;"' +
      ' title="이 번호를 끌어 장면에 놓으면 배지가 생성됩니다.">'+(i+1)+'</td>';
    ...
    tr.addEventListener('click', function(e){
      if(e.target.type!=='checkbox' && e.target.getAttribute('draggable')!=='true'){
        cb.checked=!cb.checked; toggleMaterial(m.id, cb.checked);
      }
    });
...
function selectScene(sceneIndex){
  ...
  var sceneName=appState.sceneImages[sceneIndex]?.name || ('장면 '+(sceneIndex+1));
  var titleEl=document.getElementById('currentSceneTitle');
  if(titleEl) titleEl.textContent = sceneName.replace(/\.[^/.]+$/,'');
  // 어노테이터 이미지 최신화 & 배지 렌더
  renderSceneBadges(sceneIndex);
...
function setupMinimapCanvas(){
  canvas=document.getElementById('minimapCanvas'); if(!canvas) return; ctx=canvas.getContext('2d');
  ...
  document.getElementById('resetBoxBtn')?.addEventListener('click', function(){ delete appState.minimapBoxes[appState.currentSelectedScene]; drawMinimapForScene(); });
  drawMinimapForScene();
}
...
function generatePPT(){
  ...
  document.getElementById('progress') && (document.getElementById('progress').style.display='block');
  ...
  try{
    ...
    var preview=document.getElementById('slidePreview'); if(preview) preview.innerHTML='';
    ...
    for(var i=0;i<appState.sceneImages.length;i++){
      ...
      // --- 장면 번호 배지 도형 오버레이 ---
      (function placePptBadges(){
        const marks = appState.sceneMarkers[i] || [];
        if(!marks.length) return;
        marks.forEach(mk => {
          const label = String(mk.label);
          const cx = 0.5 + (mk.x * sceneW);
          const cy = topY + (mk.y * sceneH);
          const d  = 0.26;
          slide.addText(label, {
            x: cx - d/2, y: cy - d/2, w: d, h: d,
            align:'center', valign:'middle', bold:true, fontSize:10, color:'FFFFFF',
            fill:{ color:'FF3B30' }, line:{ color:'FFFFFF', width:0 },
            shape: 'ellipse'
          });
        });
      })();
      ...
      (function placeThumbs(){
        ...
        for(var r=0; r<mats.length; r++){
          ...
          var y = tableY + rowH * (1 + r) + cellPadY;
      ...
      if(preview){
        ...
        // 미리보기 장면 위에도 배지 시각화
        var badgePreview = '';
        var marks = appState.sceneMarkers[i] || [];
        if(marks.length){
          badgePreview = '<div style="position:relative;display:inline-block;">'+
            '<img src="'+scene.data+'" style="max-width:100%;height:200px;object-fit:cover;border-radius:5px;">';
          marks.forEach(function(mk){
            badgePreview += '<div style="position:absolute;left:'+(mk.x*100)+'%;top:'+(mk.y*100)+'%;transform:translate(-50%,-50%);width:18px;height:18px;border-radius:9999px;background:#ff3b30;color:#fff;font-size:11px;display:flex;align-items:center;justify-content:center;font-weight:700;">'+mk.label+'</div>';
          });
          badgePreview += '</div>';
        }else{
          badgePreview = '<img src="'+scene.data+'" style="max-width:100%;height:200px;object-fit:cover;border-radius:5px;">';
        }

        div.innerHTML =
          '<h4 style="margin-bottom:15px;color:#2c3e50;">'+title+'</h4>'+
          '<div style="display:flex;gap:20px;align-items:flex-start;">'+
            '<div style="flex:2;">'+badgePreview+'</div>'+
    ...
    pptx.writeFile({fileName:'착공도서.pptx'}).then(function(){
      ...
      document.getElementById('previewArea') && (document.getElementById('previewArea').style.display='block');
    });
  }catch(err){
...
/* =======================
   ...
   ======================= */
// 어노테이터 DOM이 없으면 생성
function ensureAnnotatorUI(){
  if (document.getElementById('annotatorStage')) return;

  var host =
    document.getElementById('matchingStep') ||
    document.getElementById('minimapDrawWrap') ||
    document.body;

  var wrap = document.createElement('div');
  wrap.id = 'sceneAnnotator';
  wrap.style.marginTop = '14px';
  wrap.innerHTML = `
    <div style="font-weight:600;margin:6px 0 8px;">장면에 자재 번호 배치 (드래그 앤 드롭)</div>
    <div id="annotatorStage"
         style="position:relative;width:520px;height:340px;border:1px dashed #bbb;border-radius:8px;
                background:#fafafa;overflow:hidden;user-select:none;">
      <img id="annotatorImg"
           style="position:absolute;left:0;top:0;width:100%;height:100%;object-fit:contain;pointer-events:none;"
           alt="scene">
      <div id="badgeLayer" style="position:absolute;left:0;top:0;width:100%;height:100%;"></div>
      <div id="annotatorHint"
           style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);
                  color:#888;font-size:13px;background:rgba(255,255,255,.8);padding:6px 10px;border-radius:6px;">
        테이블의 <b>No.</b> 셀을 끌어다 이 영역에 놓으면 번호가 표시됩니다.
      </div>
    </div>
    <div style="display:flex;gap:8px;margin-top:8px;">
      <button id="clearSceneBadgesBtn" class="material-tab-filter-btn" type="button">이 장면 배지 모두 지우기</button>
    </div>
  `;
...
// Drag source (테이블 No. 셀)
function handleDragStart(ev, materialId, label){
  const payload = { materialId, label: String(label) };
  ev.dataTransfer.setData('application/json', JSON.stringify(payload));
  ev.dataTransfer.effectAllowed = 'copy';
}

// Drop target 세팅
function setupAnnotatorDnD(){
  ...
  stage.addEventListener('drop', function(e){
    ...
    if(!json) return;
    const { materialId, label } = JSON.parse(json);
    const rect = stage.getBoundingClientRect();
    const x = (e.clientX - rect.left) / rect.width;
    const y = (e.clientY - rect.top)  / rect.height;

    const sceneIdx = appState.currentSelectedScene || 0;
    if(!appState.sceneMarkers[sceneIdx]) appState.sceneMarkers[sceneIdx] = [];
    appState.sceneMarkers[sceneIdx].push({
      id: 'mk_'+Date.now()+'_'+Math.random().toString(36).slice(2),
      materialId: Number(materialId),
      label: label,
      x: Math.max(0, Math.min(1, x)),
      y: Math.max(0, Math.min(1, y))
    });
    renderSceneBadges(sceneIdx);
  });

  document.getElementById('clearSceneBadgesBtn')?.addEventListener('click', function(){
    const idx = appState.currentSelectedScene || 0;
    appState.sceneMarkers[idx] = [];
    renderSceneBadges(idx);
  });
}

// 배지 렌더 + 이동/삭제
function renderSceneBadges(sceneIdx){
  const imgEl = document.getElementById('annotatorImg');
  const layer = document.getElementById('badgeLayer');
  const hint  = document.getElementById('annotatorHint');
  if(!imgEl || !layer) return;

  const scene = appState.sceneImages[sceneIdx];
  if(scene) imgEl.src = scene.data;

  layer.innerHTML = '';
  const marks = appState.sceneMarkers[sceneIdx] || [];
  if(hint) hint.style.display = marks.length ? 'none' : 'block';

  marks.forEach((mk) => {
    const badge = document.createElement('div');
    badge.className = 'badge-node';
    badge.textContent = mk.label;
    badge.title = '끌어서 이동 / 클릭해서 삭제';
    badge.style.cssText = `
      position:absolute;transform:translate(-50%,-50%);
      left:${(mk.x*100).toFixed(4)}%; top:${(mk.y*100).toFixed(4)}%;
      width:22px;height:22px;border-radius:50%;
      background:#ff3b30;color:#fff;font-weight:700;font-size:12px;
      display:flex;align-items:center;justify-content:center;
      cursor:grab; box-shadow:0 1px 2px rgba(0,0,0,.25);
      user-select:none;
    `;

    let dragging=false;
    badge.addEventListener('pointerdown', (e)=>{ dragging=true; badge.setPointerCapture(e.pointerId); badge.style.cursor='grabbing'; });
    badge.addEventListener('pointerup',   (e)=>{ dragging=false; badge.releasePointerCapture(e.pointerId); badge.style.cursor='grab'; });
    badge.addEventListener('pointermove', (e)=>{
      ...
      const x = (e.clientX - rect.left)/rect.width;
      const y = (e.clientY - rect.top)/rect.height;
      mk.x = Math.max(0, Math.min(1, x));
      mk.y = Math.max(0, Math.min(1, y));
      badge.style.left = (mk.x*100)+'%';
      badge.style.top  = (mk.y*100)+'%';
    });

    badge.addEventListener('click', ()=>{
      const arr = appState.sceneMarkers[sceneIdx] || [];
      const i = arr.findIndex(a=>a.id===mk.id);
      if(i>-1){ arr.splice(i,1); renderSceneBadges(sceneIdx); }
    });
